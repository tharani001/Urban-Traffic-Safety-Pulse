...
from utils import get_prediction, ordinal_encoder

model = joblib.load('model/rf_model.pkl')

df= pd.read_csv("data/RTA_Dataset.csv")

st.set_page_config(page_title="Urban Traffic Safety Prediction App",
                   page_icon='🔮', layout="wide")

st.write("<h1 style='text-align: center; color: green'> 🔮 Urban Traffic Safety Prediction App.</h1>", unsafe_allow_html=True)

tab1, tab2 = st.tabs(['DataSet Overview', "Results"])

with tab1:
    st.write("<h4 style='color: green'>The Purpose of Project</h4>", unsafe_allow_html=True)
    
    st.write("<b>This application aims to forecast the likelihood of road traffic accidents in specific areas or along certain routes. It helps improve road safety, optimize emergency response, manage traffic flow, and inform insurance and public awareness efforts.</b>", unsafe_allow_html=True)
...
with tab2:
    st.write("<h4 style='text-align: center; color: green'> Urban Traffic Safety Prediction App</h4>", unsafe_allow_html=True)
    st.markdown('<hr>', unsafe_allow_html=True)
...
def main():
    ...
    if submit:
        ...
        st.success(f"The predicted severity is:  {pred[0]}")
